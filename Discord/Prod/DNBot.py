...
async def on_message(message):
    ...
    hardlist = ['hard like deez nuts']

    if 'dnsuggestion' in message.content.lower():
    ...
    if 'hard' in message.content.lower():
        response = random.choice(hardlist)
        await message.channel.send(response)

    if 'hard' not in message.content.lower():
        randy = random.randint(1,1000)
        if randy == 1:
            await message.add_reaction('\U0001F95C')
